...
class Functions {
	...
	*/
	public static function get_file_extension( $file_path = '' ){
		...
		$file_path = (string) parse_url( $file_path, PHP_URL_PATH );
		...
		if ( false !== strpos( $url, $dir['baseurl'] . '/' ) ) { // Is URL in uploads directory?
			...
			if ( $query->have_posts() ) {
				foreach ( $query->posts as $post_id ) {
					...
					$cropped_image_files = wp_list_pluck( (array) $meta['sizes'], 'file' );
	...
	*/
	public static function rgb_to_hex( $rgb, $default = '' ){
	  ...
	  $rgb = (string) str_replace( array(' ', 'rgba', 'rgb', '(', ')' ), '', $rgb );

	  if( preg_match( "/^[0-9]+(,| |.)+[0-9]+(,| |.)+[0-9]+$/i", $rgb ) ){
	    $rgb = str_replace( array( ',','.' ), ':', $rgb );
	    $rgbarr = explode( ':', $rgb );
	    $result = '#';
	    $result .= str_pad( dechex( (int) $rgbarr[0] ), 2, '0', STR_PAD_LEFT );
	    $result .= str_pad( dechex( (int) $rgbarr[1] ), 2, '0', STR_PAD_LEFT );
	    $result .= str_pad( dechex( (int) $rgbarr[2] ), 2, '0', STR_PAD_LEFT );
	...
	*/
	public static function hex_to_rgb( $color, $opacity = false, $default = '' ) {
	  ...
	  $color = (string) str_replace( ' ', '', $color );
	  $color = str_replace( '#', '', $color );
